...
bool PhysicalObject::detect_x_collision(const MatchMap& colition_map, const int diff) {
    ...
    for (int y = 0; y <= dimension.y; y++) {
        if (colition_map.check_collision(position.x + dimension.x + diff, position.y + y) &&
            diff > 0) {
            return true;
        }

        if (colition_map.check_collision(position.x + diff, position.y + y) && diff < 0) {
            return true;
        }
    }
...
bool PhysicalObject::detect_y_collision(const MatchMap& colition_map, const int diff) {
    ...
    for (int x = 0; x <= dimension.x && (position.x + x) < colition_map.getWidth(); x++) {
        if (colition_map.check_collision(position.x + x, position.y + diff) && diff <= 0) {
            return true;
        }
        if (colition_map.check_collision(position.x + x, position.y + dimension.y + diff) &&
            diff > 0) {
            return true;
        }
    }
...
void PhysicalObject::move(const MatchMap& colition_map) {
    ...
    if (this->speed.x != 0) {
        ...
        for (int x = 0; x <= speed.x * time_step * sign; x++) {
            if (detect_x_collision(colition_map, sign)) {
    ...
    if (speed.y <= 0) {
        for (int y = 0; y >= speed.y * time_step; y--) {
            if (detect_y_collision(colition_map, -1)) {
