...
SerialError::SerialError(const char* fmt, ...) noexcept {
    ...
    if (s < 0) {
         ...
         *
         * Lanzar una excepción no es una opción: `SerialError` es una excepción
...
const char* SerialError::what() const noexcept { return msg_error; }
