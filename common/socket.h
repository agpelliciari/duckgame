...
public:
    ...
    unsigned int recvsome(void* data, unsigned int sz);  // En caso de error -1.. Tira excepcion.

    // Send all itera hasta que, exactamente sz bytes fueron enviados. Si se envian menos queda en
    // el caller decidir
    unsigned int trysendall(const void* data, unsigned int sz);
    void sendall(const void* data, unsigned int sz);  // Intenta enviar todo. Sino tira excepcion.

    // Intenta leer lo maximo que pueda, a lo sumo sz. Retorna la cantidad leida.
    unsigned int tryrecvall(void* data, unsigned int sz);

    // Intenta leer sz bytes. Tira excepcion sino lee todo.
    void recvall(void* data, unsigned int sz);
