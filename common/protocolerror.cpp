...
ProtocolError::ProtocolError(const char* fmt, ...) noexcept {
    ...
    if (s < 0) {
         ...
         *
         * Lanzar una excepción no es una opción: `ProtocolError` es una excepción
...
const char* ProtocolError::what() const noexcept { return msg_error; }
