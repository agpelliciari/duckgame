...
int main() try {
	...
	Texture sprites(renderer, "grey_duck.png");

	// Game state
	bool is_running_right = false; // whether the character is currently running right
    bool is_running_left = false;  // whether the character is currently running left

	bool is_facing_left = false;   // whether the character is facing left
	...
	while (1) {
		...
		while (SDL_PollEvent(&event)) {
			if (event.type == SDL_QUIT) {
				...
				case SDLK_ESCAPE: case SDLK_q:
					return 0;
				case SDLK_RIGHT: is_running_right = true; is_facing_left = false; break;
				case SDLK_LEFT: is_running_left = true; is_facing_left = true; break;
				}
		...
		int src_x = 1, src_y = 10; // by default, standing sprite
		if (is_running_right || is_running_left) {
			// one of 8 run animation sprites
			src_x = 1 + 32 * run_phase;
			src_y = 10;
		}

		// Determine the flip mode based on the last direction
        SDL_RendererFlip flip = is_facing_left ? SDL_FLIP_HORIZONTAL : SDL_FLIP_NONE;

		// Draw player sprite
		renderer.Copy(
				sprites,
				Rect(src_x, src_y, 32, 32),
