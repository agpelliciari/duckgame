...
SetLobbyIdWidget::SetLobbyIdWidget(const SetLobbyIdHandler &handler, QWidget *parent) : QWidget(parent), ui(new Ui::SetLobbyIdWidget), handler(handler) {
    ui->setupUi(this);
...
SetLobbyIdWidget::~SetLobbyIdWidget() {
    delete ui;
}

void SetLobbyIdWidget::onClickJoin() {
    if (handler.onClickJoin) handler.onClickJoin();
}
