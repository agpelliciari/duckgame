...
CreateJoinWidget::CreateJoinWidget(const CreateJoinHandler &handler, QWidget *parent): QWidget(parent), ui(new Ui::CreateJoinWidget), handler(handler) {
    ui->setupUi(this);

    connect(ui->createGameButton, &QPushButton::clicked, this, &CreateJoinWidget::onClickCreateGame);
...
CreateJoinWidget::~CreateJoinWidget() {
    delete ui;
}

void CreateJoinWidget::onClickCreateGame() {
    if (handler.onClickCreateGame) handler.onClickCreateGame();
}

void CreateJoinWidget::onClickJoinGame() {
    if (handler.onClickJoinGame) handler.onClickJoinGame();
}
