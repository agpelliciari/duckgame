...
SetSoloDuoWidget::SetSoloDuoWidget(const SetSoloDuoHandler& handler, QWidget* parent):
        QWidget(parent), ui(new Ui::SetSoloDuoWidget), handler(handler) {
    ui->setupUi(this);
...
SetSoloDuoWidget::~SetSoloDuoWidget() { delete ui; }

void SetSoloDuoWidget::onClickSolo() {
    if (handler.onClickSolo)
        handler.onClickSolo();
}

void SetSoloDuoWidget::onClickDuo() {
    if (handler.onClickDuo)
        handler.onClickDuo();
}
